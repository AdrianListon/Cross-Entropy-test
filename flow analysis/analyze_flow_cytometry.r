...
stopifnot( names( fcs.channel.asinh.scale ) == fcs.channel )
...
for ( fch in fcs.channel )
    flow.data[ , fch ] <- asinh( flow.data[ , fch ] / 
        fcs.channel.asinh.scale[ fch ] )
...
# read fcs data as a flowSet
flow.set <- read.flowSet( flow.data.filename, fcs.data.dir, 
    column.pattern = paste0( fcs.channel, collapse = "|" ), 
    transformation = NULL, truncate_max_range = FALSE )

stopifnot( sort( fcs.channel ) == sort( colnames( flow.set ) ) )

flow.set <- flow.set[ , fcs.channel ]

# transform data
for ( idx in 1 : length( flow.set ) )
    for ( fch in fcs.channel )
        exprs( flow.set[[ idx ]] )[ , fch ] <- 
            asinh( exprs( flow.set[[ idx ]] )[ , fch ] / 
                fcs.channel.asinh.scale[ fch ] )

# build som objects
flow.som <- ReadInput( flow.set, transform = FALSE, scale = FALSE )
flow.som <- BuildSOM( flow.som, xdim = fcs.flow.som.dim, 
    ydim = fcs.flow.som.dim )
flow.som.mapping <- flow.som$map$mapping[ , 1 ]
flow.som.codes <- flow.som$map$codes
...
# reorder events
flow.event.cluster <- flow.som.event.cluster[ flow.event.order ]
...
for ( fs in flow.sample )
{
    sample.filename <- flow.sample.filename[ fs ]
    sample.flow.frame <- read.FCS( file.path( fcs.data.dir, sample.filename ), 
        transformation = NULL, truncate_max_range = FALSE )
    sample.event.idx <- which( flow.event.sample == fs )
    
    for ( fcg.name in names( fcs.cluster.group ) )
    {
        cluster.group <- fcs.cluster[ fcs.cluster.group[[ fcg.name ]] ]
        cluster.group.event.idx <- which( 
            ( flow.event.cluster %in% cluster.group )[ sample.event.idx ] )
        
        if ( length( cluster.group.event.idx ) > 0 )
        {
            cluster.flow.frame <- sample.flow.frame[ cluster.group.event.idx, ]
            cluster.data.dir <- sprintf( "%s/%s_%s", fcs.cluster.data.dir, 
                fcs.cluster.data, fcg.name )
            cluster.data.filename <- sprintf( "%s_%s_%s.fcs", fs, 
                fcs.cluster.data, fcg.name )
            invisible( write.FCS( cluster.flow.frame, 
                file.path( cluster.data.dir, cluster.data.filename ) ) )
        }
    }
}
